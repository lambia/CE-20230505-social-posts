...
for (let i = 0; i < likesContainer.length; i++) {
    ...
    btn.addEventListener("click", function(e) {
        ...
        //cambia classe del pulsante
        //cambia testo del numero di like
        //cambia numero di like nell'oggetto Post originale
        //pusha l'id dell'elemento in un array che contiene tutti i like
    });
}
...
function renderProPic(author) {
    let template = '';

    if(author.image != null) {
        template = `<img class="profile-pic" src="${ author.image }" alt="${ author.name }"></img>`;
    } else {
        template = `
        <div class="profile-pic-default">
            <span>${ getIniziali(author.name) }</span>
        </div>`;
    }
...
function getIniziali(autore) {
    ...
    // Versione estesa:
    // let inizialeNome = autore[0];

    // let cognome = autore.split(" ")[1]; //Seconda parte della stringa, dopo lo spazio
    // let inizialeCognome = cognome[0];
